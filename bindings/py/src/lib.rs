...
fn py_encode_chunk(py: Python, data: &[u8], ecc: u32, bits: u32) -> PyResult<String> {
    py.allow_threads(|| {
        let ecc = if ecc <= 255 {
            ecc as u8
        } else {
            return Err(PyException::new_err("ecc value is too large"));
        };
        let bits = if bits <= 255 {
            bits as u8
        } else {
            return Err(PyException::new_err("bits value is too large"));
        };
...
fn py_decode_chunk(py: Python, data: &str, ecc: u32, bits: u32) -> PyResult<DecodeOutput> {
    py.allow_threads(|| {
        let ecc = if ecc <= 255 {
            ecc as u8
        } else {
